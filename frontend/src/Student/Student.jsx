  ...
  const handleSaveStudent = async () => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        console.error('Cannot save student:', response.data);

      }
    } catch (error) {
      setLoading(false);
      console.error('Error during saving of student:', error.response ? error.response.data.message : error.message);
    }
  };
      ...
      <div className="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <div>
                  ...
                  <button type="submit" className="w-full text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800" onClick={handleSaveStudent}>Create an account</button>
                  <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                      Already have an account? <a href="/student/login" className="font-medium text-primary-600 hover:underline dark:text-primary-500">Login here</a>
                  </p>
              </div>
          </div>
      </div>
